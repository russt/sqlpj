...
main(String anArgList[])
{
    ...
    if (!sMain.check_driver()) {
        sMain.log(ERROR, sMain.PKG +
            ": JDBC driver '" + sMain.mDRIVER + "' is not available.");
        System.exit(1);
    }
...
check_driver()
{
    ...
    try {
        ...
        log(ERROR, "JDBC driver '" + mDRIVER
            + "' is not available (" + e.toString() + ").");
        return false;
    }

System.err.
printf("check_driver: this.mURL='%s' this.mUSER='%s' this.mPASSWORD='%s' this.mDRIVER='%s' Class.forName='%s'\n",
mURL, mUSER, mPASSWORD, this.mDRIVER, xx);
 ...
 */
private String
sql_results_to_string(java.sql.ResultSet aResultSet)
{
    try {
        if (aResultSet == null) {
            //this is an error
...
sql_eoi(String aLine)
{
    ...
    log(DEBUG, "sql_eoi returned "
        + (res ? "true" : "false") + " aLine='"
        + (aLine == null ? "NULL" : aLine) + "'");
...
{
    boolean showit =    ( mDEBUG & (aLevel >= DEBUG) )
                        | ( mVERBOSE & (aLevel >= INFO) )
                        | ( mSHOWERRORS & (aLevel == ERROR) );
...
"delimiters are provided, then statements are executed at EOF.",
"",
"Options:",
" -help          display this help message",
" -verbose       display information messages",
" -debug         display debug messages",
" -q             quiet mode.  do not display prompt if reading from stdin",
" -prompt string use <string> as prompt instead of default '" + sMain.mPROMPT + "'.",
"",
...
"Example:",
"",
" " + PKG + " -url \"jdbc:oracle:thin:@pipit.sfbay.sun.com:1521:FORTE\"",
...
parse_args(String anArgList[])
{
    ...
    while (ii <  anArgList.length) {
        ...
        if (arg.startsWith("-h")) {
            ...
            if (sMain.mURL.startsWith("-")) {
                sMain.log(ERROR, PKG +
                    ":  -url parameter '" + sMain.mURL + "' is invalid.");
                return false;
            }
            ...
            if (sMain.mDRIVER.startsWith("-")) {
                sMain.log(ERROR, PKG +
                    ":  -driver parameter '" + sMain.mDRIVER + "' is invalid.");
                return false;
            }
            ...
            if (sMain.mUSER.startsWith("-")) {
                sMain.log(ERROR, PKG +
                    ":  -user parameter '" + sMain.mUSER + "' is invalid.");
                return false;
            }
            ...
            if (sMain.mPROMPT.startsWith("-")) {
                sMain.log(ERROR, PKG +
                    ":  -prompt parameter '" + sMain.mPROMPT + "' is invalid.");
                return false;
            }
            ...
            if (sMain.mPASSWORD.startsWith("-")) {
                sMain.log(ERROR, PKG +
                    ":  -password parameter '" + sMain.mPASSWORD + "' is invalid.");
                return false;
            }
    ...
    if (sMain.mURL == null || sMain.mDRIVER == null || sMain.mUSER == null) {
        sMain.log(ERROR, PKG
            + ":  missing a required parameter (-user, -driver, or -url)");
        return false;
    }
    ...
    //allocate array big enough to hold non-empty filenames
    sFILES = new String[anArgList.length - (ii + nempty)];

    //collect non-empty file args:
    int jj = 0;
    while (ii < anArgList.length) {
    ...
    if (sMain.mVERBOSE) {
        sMain.mDOPROMPT = true;    //normally we supress prompts for batch execution
    }
 ...
 *
 */
final private static String PKG = "Jsql";
 ...
 */
public
Jsql(String aUrl, String aDriver, String aUser, String aPassword)
{
    mURL = aUrl;
    mDRIVER = aDriver;
    mUSER = aUser;
